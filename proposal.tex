...
\begin{document}
	...
	\begin{enumerate}
		...
		\item The minimizing distance problem for a specific team is similar to traveling salesman problem - could try to model this problem in form of A* and find a heuristic based on number of divisional games remaining (divisional games are regionally closer) - then rearrange schedule to fit that team's path. We could try to first accomplish this but the goal is to minimize the combined distance for every team traveled which will require outside research to see if this is even feasible. At a minimum we will try to beat the total distance traveled from the actual NBA schedule for this year through a greedy approach of finding minimum distance traveled while optimizing for 1 team, then minimum distance traveled for for 2 teams including the best from the 1-team, and so on until as many teams as possible have minimized their travel distance.
